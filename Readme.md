...
**快门速度：** 3s-1/8000s，共 27 档，支持 B 门及 T 门 。 T 门可设置时间，至多5h （非定时T和B门无时间限制）\
**自拍定时：** 0s/2s/5s/10s （倒计时完成后按当前模式自动曝光，倒计时期间挂起其余 Core 1 任务，不受按键干扰）\
**多重曝光：** 最多 11 张（受 MCU 机能限制，曝光过程中无法折叠相机，断电将导致快门保持打开状态造成严重过曝）。多重曝光期间电磁铁持续通电，间隔超过 5 分钟可能导致过热，建议尽快完成所有曝光并吐片。设置方式：长按进入设置菜单，切换 MultiExp 选项（-1 为仅中止/首张防护，0 为正常拍摄，1-10 为额外曝光张数）\
...
| `gpio_inputs.c` | GPIO 输入处理：S1/S2 去抖、3D 按键、设置菜单（自拍定时 / 多重曝光 / IP 显示） |
| `display_manager.c` | OLED 帧绘制、倒计时显示、设置菜单布局 |
| `opt4001.c` | OPT4001 环境光传感器 I2C 驱动 |
| `ssd1306.c` | SSD1306 OLED I2C 驱动（esp_lcd 后端，含 `ssd1306_draw_str_black` 反色绘制） |
...
长按 3D 按键的"按下"键进入设置菜单，菜单参数化支持上下键切换选项、短按"按下"键切换调整/浏览模式：

| 选项 | 说明 |
|------|------|
| **SelfTimer** | 自拍定时：0s / 2s / 5s / 10s |
| **MultiExp** | 多重曝光：-1（仅中止，首张防护）/ 0（正常）/ 1-10（额外张数，最多 11 张）|
| **IP Addr** | 只读，显示相机当前 WiFi IP 地址（未连接时显示 "not connected"）|

状态栏右上角同时显示当前自拍定时值与多重曝光剩余张数。
...
2. 手机或电脑连接同一局域网，浏览器打开相机 IP（OLED 设置菜单中可查看）
3. 选择编译好的 `.bin` 固件文件上传
